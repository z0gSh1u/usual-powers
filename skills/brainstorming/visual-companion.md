...
The script detects common hosted environments. If your host reaps detached
processes, add `--foreground` and launch it with that host's background-process
mechanism so it survives across turns. The exact mechanism is platform-specific;
do not copy another host's tool arguments blindly.
